...
size_t SparseImgAlignGpu::run(FramePtr ref_frame, FramePtr cur_frame, FILE* log)
{
  ...
  cl_float error[feature_counter_];
  cl_float chi[feature_counter_];
  residual_->load(1,10,feature_counter_,chi);
  residual_->load(1,7,feature_counter_,error);
...
{
    cl_float H[9*feature_counter_];
    cl_float3 J[feature_counter_];
    cl_float chi2_[feature_counter_];
