    ...
    const double timestamp)
{
  ...
  if(pub_pose_with_cov_.getNumSubscribers() > 0)
  {
    ...
    geometry_msgs::PoseWithCovarianceStampedPtr msg_pose_with_cov(new geometry_msgs::PoseWithCovarianceStamped);
      ...
      msg_pose_with_cov->pose.pose.position.x = -1.0*odom.second.se2().translation()(0);
      msg_pose_with_cov->pose.pose.position.y = -1.0*odom.second.se2().translation()(1);
    ...
    if(pub_frames_.getNumSubscribers() > 0 || pub_points_.getNumSubscribers() > 0){
          publishPointMarker(
                  pub_points_, Eigen::Vector3d(-1.0*odom.second.se2().translation()(0),-1.0*odom.second.se2().translation()(1),0.0), "trajectory",
