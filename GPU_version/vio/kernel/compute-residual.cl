...
void compute_Jacobian(double3 j, __global double* Jacobian, double r, double w)
{
    *(Jacobian + 0) -= j[0] * r * w;
    *(Jacobian + 1) -= j[1] * r * w;
    *(Jacobian + 2) -= j[2] * r * w;
...
)
{
    ...
    double3 xyz_current = xyz_cur(cur_pose[0], cur_pose[1], cur_pose[2], ref_pose[0] ,ref_pose[1] ,ref_pose[2], ref_feature[f]);
    ...
    for(int y = 0; y < PATCH_SIZE; ++y)
    {
        ...
        // compute residual
        int2 px_curtl = ( cur_element_addr                                      % get_image_dim(image_cur)[0],  cur_element_addr                                      / get_image_dim(image_cur)[0]);
        int2 px_curtr = ((cur_element_addr + 1)                                 % get_image_dim(image_cur)[0], (cur_element_addr + 1)                                 / get_image_dim(image_cur)[0]);
        int2 px_curbl = ((cur_element_addr + (get_image_dim(image_cur)[0]))     % get_image_dim(image_cur)[0], (cur_element_addr + (get_image_dim(image_cur)[0]))     / get_image_dim(image_cur)[0]);
        int2 px_curbr = ((cur_element_addr + (get_image_dim(image_cur)[0]) + 1) % get_image_dim(image_cur)[0], (cur_element_addr + (get_image_dim(image_cur)[0]) + 1) / get_image_dim(image_cur)[0]);
        ...
        errors[(int)(f * pow((double)PATCH_SIZE, 2.0) + (y * PATCH_SIZE) + x)] = fabs(res);
