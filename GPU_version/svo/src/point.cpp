...
void Point::initNormal()
{
  ...
  normal_ = ftr->frame->T_f_w_.T->rotation_matrix().transpose()*(-ftr->f);
...
void Point::optimize(const size_t n_iter)
{
  ...
  for(size_t i=0; i<n_iter; i++)
  {
    ...
    {
      Matrix23d J;
      const Vector3d p_in_f(pos_.x()*(*it)->frame->T_f_w_.cos_theta_+pos_.z()*(*it)->frame->T_f_w_.sin_theta_+(*it)->frame->T_f_w_.X_,
                 pos_.y(),
                 -pos_.x()*(*it)->frame->T_f_w_.sin_theta_+pos_.z()*(*it)->frame->T_f_w_.cos_theta_+(*it)->frame->T_f_w_.Z_);
      Point::jacobian_xyz2uv(p_in_f, (*it)->frame->T_f_w_.T->rotation_matrix(), J);
